...
class CalendarList extends Component {
    ...
    renderCalendar({item}) {
        ...
        if (item.getTime) {
            let date = new Date();
            date.setFullYear(item.getFullYear());
            date.setMonth(item.getMonth());
            date.setDate(1);
            let fday = date.getDay();
            let days = new Date(item.getFullYear(), item.getMonth()+1, 0).getDate();
            rows = Math.ceil((fday + days) / 7);
        }
        //动态根据行数修改的Canlendar整体高
        return (<CalendarListItem item={item} calendarHeight={calendarHeight + (rows - 5) * 63} {...this.props} />);
    }
